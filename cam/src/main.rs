...
async fn main() -> std::io::Result<()> {
    ...
    let app_state = web::Data::new(AppState {
        app_name: "CAM server".to_string(),
        db: setup_db().await.unwrap(),
        celery_app: None,
    });

    #[utoipauto(paths = "./cam/src,./scmscommon/src from scmscommon")]
    #[derive(OpenApi)]
    #[openapi(info(title = "Certificate Access manager (CAM)"))]
    struct OpenApiDoc;
    ...
    .bind(("0.0.0.0", global_config.cam_port))
    .unwrap()
    .run()
    .await
}
