...
async fn healthcheck() -> impl Responder {
    let la_id = LaConfig::global().la_id;

    HttpResponse::Ok().body(format!("LA{la_id} Server is healthy\n"))
}
...
async fn main() -> std::io::Result<()> {
    ...
    load_mysql_config();
    laconfig::load_la_config();

    let app_state = web::Data::new(AppState {
        app_name: "LA server".to_string(),
    ...
    // Load LA Certificates
    load_la_certificates_keys::load_certificates_to_database(&app_state.db)
        .await
        .unwrap();

    #[utoipauto(paths = "./la/src,./scmscommon/src from scmscommon")]
    #[derive(OpenApi)]
    #[openapi(info(title = "Linkage Authority (LA)"))]
    struct OpenApiDoc;
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(endpoint::get_plv_to_ls_map)
            .service(
